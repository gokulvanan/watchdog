...
module.exports = function(){
    ...
    function createAndBindEventSourceToPlugins(val,key,clusterName){
        var es = new Eventsource(val.url);
    ...
    function transformClusters(val,key){
        ...
        for(var j=0; j<val.length; j++){
            ...
            for(var i = obj.start; i <= obj.stop; i++){
                ...
                output[obj.prefix+count] = {
                    url: "http://"+obj.prefix+count+"."+obj.suffix+":"+obj.port+obj.path,
                    port:obj.port
                };
            }
        }
    ...
    function buildHttpServer(conf,es,plugins){
        ...
        }).listen(conf.port, '0.0.0.0'); //0.0.0.0 added to be accessible from other boxes
