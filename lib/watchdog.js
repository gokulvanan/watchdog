...
(function(){
  ...
  program
    .version('0.0.1')
    .option('-c, --config [cofig_path]', 'path to config.js',process.cwd()+"/config.js")
  ...
  function showStatus(){
    ...
    var pids = fh.getPids(require(program.config).pid);
  ...
  function clean(){
      ...
      var loggerConf = require(program.config).logger;
  ...
  function start(){
    ...
    if (fs.existsSync(pidFile)){
      ...
      var bg = require("child_process").fork(__dirname+"/bootstrap.js",
        [program.config],"/usr/bin/env node");// run server as separate process
  ...
  function stop(){
      ...
      var pidFile = require(config).pid;
  ...
  function errorMsg(){
    console.log("Oops.. valid config not found in default path "+program.config);
    console.log("");
